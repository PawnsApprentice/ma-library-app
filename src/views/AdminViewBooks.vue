<template>
  <div class="flex flex-wrap">
    ...
    <div v-if="!storeLibrary.booksLoaded" class="w-full">
      <a-spin tip="Loading...">
        <a-alert message="Retrieving Data" description="Please Hold"></a-alert>
      </a-spin>
    </div>
    <!--       MODAL FOR NOTIFY BUTTON     -->
    <a-modal
      v-model:visible="modalNotifyVisible"
      title="Notify User"
      @ok="notifyHandleOk"
        ...
        >
          ...
          <a-form-item
            ...
            :rules="[
              {
                required: true,
                message: 'Please input the name of the author!',
              },
            ]"
          >
          ...
          <a-form-item
            ...
            :rules="[
              {
                required: true,
                message: 'Please input the url for the book image',
              },
            ]"
          >
    ...
    <div class="w-full">
      ...
      <div
        ...
        class="flex justify-between w-full px-12 pt-10"
      >
        <div>
          <a-dropdown>
            <template #overlay>
        ...
        <a-button title="Add Book" type="primary" @click="showAddDrawer">
          <template #icon>
        ...
        <template #default="{ record }">
          <span>
            ...
            <template v-if="record.status === 'AVAILABLE'">
              <a-divider type="vertical" />
              <a-button type="primary" @click="checkoutBook(record)"
                >Checkout</a-button
              >
            </template>
            <template
              v-if="
                record.loaned_by === currentUser && record.status === 'RESERVED'
              "
            >
              <a-divider type="vertical" />
              <a-button type="primary" @click="returnBook(record)"
                >Return</a-button
              >
            </template>
            <template
            ...
            >
              <a-divider type="vertical" />
              <a-button type="primary" @click="notifyUser(record)"
                >Notify</a-button
              >
            </template>
          </span>
...
async function onSearch(searchtext) {
  ...
  debounce.value = setTimeout(async () => {
    ...
    if (booksJsonArr.value.length > 1) {
      ...
      options.value = [
        { value: booksJsonArr.value[0].title },
        { value: booksJsonArr.value[1].title },
        { value: booksJsonArr.value[2].title },
      ];
    }
  }, 1300);
}
...
function onFinishEdit() {
  storeLibrary.editBooks(
    refBookSelected.value.id,
...
function filterUserBooks() {
  if (buttonDropdownSelected.value !== "Filter By") {
    storeLibrary.getFilteredUserBooks(
      filteredUser.value,
      buttonDropdownSelected.value
    );
...
function handleDropdownStatusClick(e) {
  ...
  storeLibrary.getBooksByStatus(
    buttonDropdownSelectedStatus.value,
    buttonDropdownSelected.value,
    filteredUser.value
  );
}
...
async function findBookInfo() {
  if (formState.bookTitle.length > 3) {
    ...
    await fetch(
      `https://openlibrary.org/search.json?q=${formState.bookTitle}&limit=3`
    )
      .then((a) => a.json())
      .then((response) => {
        for (let i = 0; i < 3; i++) {
          ...
          let temp = {
            ...
            image:
              "https://covers.openlibrary.org/b/isbn/" + tempIsbn + "-M.jpg",
          };
